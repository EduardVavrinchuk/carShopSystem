...
library StorageInterface {
    ...
    bytes32 constant CAR_IDENTIFIER = "car";
    ...
    struct Bytes32CarMapping {
      Car innerMapping;
    }

    struct UIntCarMapping {
      Bytes32CarMapping innerMapping;
    }
    ...
    function init(Car storage self, bytes32 _id) internal {
      init(self.brand, keccak256(_id, 'brand'));
      init(self.producer, keccak256(_id, 'producer'));
    ...
    function get(Config storage self, Car storage item, bytes32 _key) internal constant returns (bytes32[2] res, bool res1, uint[2] res2, address owner) {
        res[0] = get(self, item.brand, _key);
        res[1] = get(self, item.producer, _key);
    ...
    function toBool(bytes32 self) public constant returns(bool) {
        return self != bytes32(0);
    }

    function toBytes32(bool self) public constant returns(bytes32) {
